...
def to_madrid(series: pd.Series) -> pd.Series:
    ...
    Maneja: naive, UTC, y ya localizadas.
    """
...
def global_metrics() -> Dict[str, Any]:
    ...
    df = qry("""
        SELECT
            COUNT(*)                                                                AS total,
            COUNT(*) FILTER (WHERE COALESCE(pnl, pnl_usd) > 0)                    AS wins,
            COALESCE(SUM(COALESCE(pnl, pnl_usd)) FILTER
                (WHERE COALESCE(pnl, pnl_usd) > 0),  0)                           AS gross_win,
            COALESCE(ABS(SUM(COALESCE(pnl, pnl_usd))) FILTER
                (WHERE COALESCE(pnl, pnl_usd) <= 0), 0)                           AS gross_loss,
...
last_ping = pd.to_datetime(hb["last_ping"].iloc[0]) if not hb.empty else None
...
with t1:
    ...
    if df_open.empty:
        ...
        st.dataframe(
            df_od.style.applymap(_cpnl, subset=["PnL no real."]),
...
with t2:
    ...
    if rows2:
        ...
        st.dataframe(
            df_pf.style.applymap(_cpf, subset=["PnL USD","PnL %"]),
            use_container_width=True, hide_index=True,
        )
...
with t4:
    ...
    with col_t:
        ...
        if df_mo2.empty:
            ...
            st.dataframe(
                df_rm.style.applymap(_cm, subset=["PnL USD","PnL %"]),
                use_container_width=True, hide_index=True,
            )
        ...
        if df_cl.empty:
            ...
            st.dataframe(
                df_cl.style.applymap(_cc, subset=["pnl_usd"]),
                use_container_width=True, hide_index=True,
            )
...
with t6:
    ...
    if not frames:
        ...
        df_fiscal["fecha"] = pd.to_datetime(df_fiscal["fecha"], errors="coerce")
        # Convertir a Madrid para fiscal
        df_fiscal["fecha"] = to_madrid(df_fiscal["fecha"]).dt.date
        df_fiscal = df_fiscal.sort_values("fecha")

        # Métricas fiscales del año actual
        year_now = date.today().year
        df_year  = df_fiscal[pd.to_datetime(df_fiscal["fecha"]).dt.year == year_now]
