...
if last_ping:
    since = (datetime.now(timezone.utc) - pd.to_datetime(last_ping).tz_localize(
        timezone.utc if pd.to_datetime(last_ping).tzinfo is None else None
    )).total_seconds() / 60
    ...
    if portfolio_rows:
        ...
        def style_pnl(val):
            if isinstance(val, (int, float)):
                color = "#1D9E75" if val > 0 else ("#E24B4A" if val < 0 else "gray")
                return f"color: {color}; font-weight: 500"
            return ""

        styled = df_pf.style.applymap(style_pnl, subset=["PnL (USD)", "PnL %"])
...
with tab_bot:
    ...
    if df_trades.empty:
        ...
        styled_trades = df_show.style.applymap(style_pnl_row, subset=["pnl_usd"] if "pnl_usd" in df_show.columns else [])
...
with tab_manual:
    ...
    with col_form:
        ...
        with st.form("form_add_manual", clear_on_submit=True):
            m_sym   = st.text_input("Par (ej. BTC/USDT, SOL/USDT)").upper().strip()
    ...
    with col_table:
        ...
        if manual_open_now.empty:
            ...
            st.dataframe(
                df_m.style.applymap(color_pnl_cell, subset=["PnL USD", "PnL %"]),
...
with tab_validation:
    ...
    if not all_trades_v.empty and "entry_time" in all_trades_v.columns:
        ...
        weeks_in_paper = (datetime.now(timezone.utc) - first.tz_localize(timezone.utc)).days / 7
