...
def load_daily_snapshot(symbol: str, as_of_date: pd.Timestamp) -> pd.DataFrame:
    ...
    Usado para que el CycleDetector no vea el futuro en el backtest.
    """
    conn = sqlite3.connect(SQLITE_PATH)
    df = pd.read_sql_query(
        ...
        conn,
        params=(symbol, int(as_of_date.timestamp()*1000))
    )
    conn.close()
    if not df.empty:
        df["timestamp"] = pd.to_datetime(df["timestamp"], unit="ms", utc=True)
...
def run_backtest(
    ...
    # Ciclo macro inicial (se actualiza cada 24 velas)
